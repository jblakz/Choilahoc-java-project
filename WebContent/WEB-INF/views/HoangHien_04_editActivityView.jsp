...
<head>
	...
	<title>ChoiLaHoc.vn - Edit Post</title>
...
<body>
	...
	<div id="body">
			...
			<div class="footer">
				<div class="contact">
					<h1>Chỉnh sửa thông số bài đăng hoạt động</h1>
					<p style="color: red;">${errorString}</p>
					<form method="POST" action="doEditActivity" enctype="multipart/form-data">
						<input type="hidden" name="idActivities" value="${activity.idActivities}" />
						<input type="text" name="title" value='${activity.title}'
						onblur="this.value=!this.value?'${activity.title}':this.value;" onfocus="this.select()">
						<select name="idCategory">
							<c:forEach items='${categoriesList}' var="category" >
								<option value="${category.idCategory}" id = "catOption_${category.idCategory}">
									${category.catName}
								</option>
							</c:forEach>
						</select>
						<textarea name="content" id="editor1" cols="50" rows="7">${activity.content}</textarea>
						<input style="display:inline;"
						type="submit" value="Cập nhật" id="submit">
						<input style="display:inline;"
				...
				<div class="section" style="margin-top:68px;">
					...
					<img src="${pageContext.request.contextPath}/imagesURL/${activity.idActivities}" alt="Image File"
						style="width:180px; height: 180px; border-radius: 50%;object-fit: cover; display:block;margin:auto;"
						id="thumb">
				</div>
				<button class="btn2" onclick="location='${pageContext.request.contextPath}/manage'">Quay lại</button>
			</div>
		</div>
		...
		window.onload = function(){
			var categoryId = '${activity.idCategory}';
